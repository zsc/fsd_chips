...
2021年之前，汽车电子系统采用分布式ECU架构，一辆高端车型可能包含超过100个ECU，分别控制不同功能：
...
└─────────────────────────────────────────────────────────────┘
问题：
• 线束复杂度高：总重量可达60kg，成本超过3000美元
• 数据孤岛：各ECU独立处理，难以实现传感器融合
• 算力浪费：每个ECU算力利用率不足30%
• OTA困难：需要逐个更新ECU固件
```
...
#### 热管理挑战
- 域控制器功耗显著增加（200W~500W）
- 采用液冷散热系统
- 动态功耗管理（DVFS）策略

### 3.1.4 主要玩家布局

| 厂商 | 域控制器平台 | 芯片方案 | 首发车型 | 量产时间 |
|------|------------|---------|---------|---------|
| 博世 | DASy 4.0 | Mobileye EyeQ5 | 大众ID.4 | 2021 Q2 |
| 大陆 | ADCU | NVIDIA Xavier | 蔚来ET7 | 2022 Q1 |
| 德赛西威 | IPU04 | 地平线征程5 | 理想L9 | 2022 Q2 |
| 华为 | MDC 610 | 昇腾610 | 极狐αS HI | 2021 Q4 |
| 采埃孚 | ProAI | NVIDIA Orin | 奔驰EQS | 2022 Q3 |
...
地平线BPU采用创新的贝叶斯架构，专为视觉感知优化：

#### 核心创新点

1. **稀疏计算优化**
   - 动态稀疏度感知：根据网络层特性自适应调整
   - 结构化剪枝支持：2:4、4:8稀疏模式
   - 实测加速比：相比密集计算提升3.2倍
